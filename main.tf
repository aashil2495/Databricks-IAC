...
resource "databricks_storage_credential" "azure_access_connector_credential" {
  name = "external_loc_cred"
...
resource "databricks_external_location" "external_location" {
  ...
  url                  = "abfss://metastore-root@terraformfitbitadlsg2dev.dfs.core.windows.net/"
  credential_name = databricks_storage_credential.azure_access_connector_credential.id
}

resource "databricks_cluster" "personal_compute" {
  cluster_name            = "Ashil's Personal Compute"
  spark_version           = "13.3.x-scala2.12" # Adjust as needed
  node_type_id            = "Standard_DS3_v2"  # Use an instance type suitable for your workload
  autotermination_minutes = 10                # Auto-terminate after inactivity
  autoscale {
    min_workers = 1
    max_workers = 2
  }
  ...
  single_user_name = "ashil@itmiracle.com" # Replace with your Databricks user email
}
